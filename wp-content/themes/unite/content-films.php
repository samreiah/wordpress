...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
        ...
        ?>
        <h4>Ticket Price: <small><?php echo (isset($customFields['ticket_price']) && !empty($customFields['ticket_price'])) ? $customFields['ticket_price'][0] : 'Not Added yet' ?></small></h4>
        
        <h4>Released on: <small><?php echo (isset($customFields['released_on']) && !empty($customFields['released_on'])) ? $customFields['released_on'][0] : 'Not Added yet' ?></small></h4>
        
        
        <h4> Country/Countries : 
            
            <?php $countries = wp_get_post_terms( get_the_ID(), 'Country' ); ?>

            <?php 

                if(empty($countries)) {
                    echo '<small>No Countries Configured</small>';
                } else { foreach($countries as $country)
                    echo '<small>'. $country->name .'</small>';
                }
        ...
        <h4> Genres : 
            ...
            <?php 
                ...
                if(empty($genres)) {
                    echo '<small>No Countries Configured</small>';
                } else { foreach($genres as $genre)
                    echo '<small>'. $genre->name .'</small>';
                }
